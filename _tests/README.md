...
Run from the project root. All three need the `quarto` binary and the Deno it bundles, and none
works inside a musl/Alpine sandbox — see the sandbox note in `CLAUDE.md`.

## Why `quarto run` hosts a `deno test` subprocess

`quarto run` shells out to `deno run`. That discards `Deno.test()` registrations silently — no
warning, exit 0 — and, more importantly, never type-checks. The second half is not theoretical: two
`TS2339` errors reading a deleted `UnadoptedDoc.line` once composed the literal string
`_isms.yml:undefined` into the deviations register, which is the entire audit record for an
un-adopted document, and every tool stayed green.
...
| Tier | Needs | What it asks |
|---|---|---|
| `blocks_test.ts` | nothing | Does the block grammar enforce its rules and refuse what it says it refuses? |
| `compose_test.ts` | nothing | The override contract, the front-matter merge table, the asset walk. |
| `adoption_test.ts` | nothing | Every `_isms.yml` validation path. |
| `deviations_test.ts` | nothing | What the register reports, and where its deep links land. |
| `cli_test.ts` | nothing | `isms.ts` as a subprocess: write-only-on-change, prune, exit codes. |
| `tree_test.ts` | `quarto render` | Does what is on disk agree with the composer? |
| `site_test.ts` | `quarto render` | Did Quarto publish exactly what the register says it did? |

The first five build throwaway projects in temp dirs — `compose()` is read-only and takes its root
from the `Project` it is handed, so a complete fixture is three files and a full compose runs in
about 15 ms with no Quarto process.

The last two are a different question, and not a substitute: a fixture cannot notice that someone
hand-edited `docs/`, forgot to render, or dropped a document from the manifest without pruning its
output. Both refuse to run rather than pass vacuously when there is nothing on disk to disagree
with.
